...
public class DataInputFragment extends Fragment {
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        mAddMealButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                Meal mealToSend = new Meal();
                mealToSend.calories = Double.parseDouble(mMealCaloriesEditText.getText().toString());
                mealToSend.carbs = Double.parseDouble(mMealCarbsEditText.getText().toString());
                mealToSend.cholest = Double.parseDouble(mMealCholestEditText.getText().toString());
                mealToSend.fat = Double.parseDouble(mMealFatEditText.getText().toString());
                mealToSend.fiber = Double.parseDouble(mMealFiberEditText.getText().toString());
                mealToSend.protein = Double.parseDouble(mMealProteinEditText.getText().toString());
                mealToSend.sodium = Double.parseDouble(mMealSodiumEditText.getText().toString());
                mealToSend.sugars = Double.parseDouble(mMealSugarsEditText.getText().toString());

                // TODO: Add to datetime to parse to custom date and time format.
                int month = mMealDatePicker.getMonth() + 1; // Java Months are 0-indexed
     ...
     * */
    private void sendMealDataToServer(Meal meal){
        ...
        String mealMapString = gson.toJson(meal);
    ...
    private class PostMealDataTask extends AsyncTask<String, Integer, String>{
        ...
        protected String doInBackground(String... strings) {
            ...
            URL url = null;
            try {
                url = new URL("http://104.236.167.62:5000/input/meal");
            ...
            return "Server Response < 400 OK!";
        }

        protected void onPostExecute(String result) {
            if (result != null) {
                Toast.makeText(getContext(), String.format("Successfully sent meal to server! Result: %s", result), Toast.LENGTH_LONG).show();
            } else {
                Toast.makeText(getContext(), "Error posting to server.", Toast.LENGTH_SHORT).show();
            }
        }
